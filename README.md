...
- **Lag-trend sparklines** — each replica card draws a rolling history of its lag, so you can see whether it's holding steady or **drifting**, not just the instant value.
- **Threshold alerts** — opt-in desktop notifications fire the moment a node goes **unreachable** or its lag turns **critical**, even when the panel isn't in focus.
...
- **Managed-cluster detection** — nodes on **Amazon RDS / Aurora**, **MongoDB Atlas**, **repmgr** or MySQL Group Replication are detected and badged (the groundwork for safely refusing future topology actions on orchestrated clusters).
...
- **✦ AI advisor** — one click hands the live snapshot to your configured AI provider (Claude · Gemini · Mistral · Grok · Ollama), which **explains what's wrong, the likely cause and the exact *advisory* commands** to investigate. It diagnoses; you decide and run — and it won't recommend a manual failover on a managed cluster.
- Read-only **monitoring & advice** by design — DataDock executes nothing and has no promotion/failover buttons (issuing those directly would fight orchestrators like Patroni/RDS); the focus is seeing and understanding your topology clearly and safely.
...
| 🌐 **Replication topology** | Group connections into a replication set with primary/replica/arbiter roles · **live topology view** with lag-coloured links & node-health badges · **lag-trend sparklines** · **threshold desktop alerts** (node down / critical lag) · **managed-cluster detection** (RDS · Aurora · Atlas · repmgr · Group Replication) · **✦ AI advisor** (diagnoses lag/health + advisory commands, refuses manual failover on managed clusters) · normalized per-engine status (Postgres · MySQL · MongoDB · Redis) · privilege-aware "no status" + role-mismatch warnings · auto-refresh · click-through to any node — read-only monitoring & advice (no failover) |
